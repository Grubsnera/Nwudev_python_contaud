...
so_file = "Kfs_vss_studdeb_2017.sqlite" #Source database
...
funcfile.writelog("%t OPEN DATABASE: Kfs_vss_studdeb_2017")
...
SELECT
  *,
  CASE
    WHEN ACCOUNT_NBR = '1G00362' THEN 1
    WHEN ACCOUNT_NBR = '1G00963' THEN 2
    WHEN ACCOUNT_NBR = '1G00265' THEN 9
    WHEN ACCOUNT_NBR = '1G00267' THEN 9
    ELSE 0
  ...
  END AS STUDENT
FROM
  KFS.X000_GL_trans_2017
WHERE
  (KFS.X000_GL_trans_2017.FIN_OBJECT_CD = '7551') OR
  (KFS.X000_GL_trans_2017.FIN_OBJECT_CD = '7552') OR
  (KFS.X000_GL_trans_2017.FIN_OBJECT_CD = '7553')
;"""
so_curs.execute("DROP TABLE IF EXISTS "+sr_file)
so_curs.execute(s_sql)
so_conn.commit()
...
;"""
#s_sql = s_sql.replace("%PYEAREND%",'2016-12-31')
...
FROM
  X002ab_vss_transort
WHERE
  X002ab_vss_transort.TRANSDATE_VSS > Date('%CYEAREND%')
;"""
#s_sql = s_sql.replace("%CYEAREND%",funcdate.cur_yearend())
s_sql = s_sql.replace("%CYEAREND%",'2016-12-31')
so_curs.execute("DROP TABLE IF EXISTS "+sr_file)
so_curs.execute(s_sql)
so_conn.commit()
funcfile.writelog("%t BUILD TABLE: "+sr_file)
...
so_curs.execute("DROP TABLE IF EXISTS "+sr_file)
s_sql = s_sql.replace("%CYEAR%",'2017')
s_sql = s_sql.replace("%PMONTH%",gl_month)    
so_curs.execute(s_sql)
so_conn.commit()
funcfile.writelog("%t BUILD TABLE: "+sr_file)
