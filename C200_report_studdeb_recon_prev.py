...
funcfile.writelog("%t BUILD TABLE: "+sr_file)
# Export the data
print("Export gl student debtor summary per month...")
sr_filet = sr_file
sx_path = re_path + funcdate.prev_year() + "/"
sx_file = "Debtor_001_glsummmonth_"
sx_filet = sx_file + funcdate.prev_monthendfile()
s_head = funccsv.get_colnames_sqlite(so_conn, sr_filet)
funccsv.write_data(so_conn, "main", sr_filet, sx_path, sx_file, s_head)
#funccsv.write_data(so_conn, "main", sr_filet, sx_path, sx_filet, s_head)
...
s_sql = "CREATE TABLE "+sr_file+" AS " + """
SELECT
  UPPER(SUBSTR(X002cb_vss_balmonth.CAMPUS_VSS,1,3))||TRIM(X002cb_vss_balmonth.MONTH_VSS) AS ROWID,
  'NWU' AS ORG,
  X002cb_vss_balmonth.CAMPUS_VSS AS CAMPUS,
  X002cb_vss_balmonth.MONTH_VSS AS MONTH,
  X002cb_vss_balmonth.AMOUNT_DT AS VSS_TRAN_DT,
  X002cb_vss_balmonth.AMOUNT_CT AS VSS_TRAN_CT,
  X002cb_vss_balmonth.AMOUNT AS VSS_TRAN,
  X001cb_gl_balmonth.BALANCE AS GL_TRAN
FROM
  X002cb_vss_balmonth
...
WHERE
  X002cb_vss_balmonth.MONTH_VSS <= '%PMONTH%'
;"""
so_curs.execute("DROP TABLE IF EXISTS "+sr_file)
...
s_sql = "CREATE TABLE " + sr_file + " AS " + """
SELECT
  X002ea_vss_gl_balance_month."ROWID",
  X002ea_vss_gl_balance_month.ORG,
  X002ea_vss_gl_balance_month.CAMPUS,
  X002ea_vss_gl_balance_month.MONTH,
  X002ea_vss_gl_balance_month.VSS_TRAN_DT,
  X002ea_vss_gl_balance_month.VSS_TRAN_CT,
  X002ea_vss_gl_balance_month.VSS_TRAN,
  X002ea_vss_gl_balance_month.GL_TRAN,
...
FROM
  X002ea_vss_gl_balance_month
  LEFT JOIN X002eb_impo_report_officer CAMP_OFFICER ON CAMP_OFFICER.CAMPUS = X002ea_vss_gl_balance_month.CAMPUS
  LEFT JOIN X002eb_impo_report_officer ORG_OFFICER ON ORG_OFFICER.CAMPUS = X002ea_vss_gl_balance_month.ORG
  LEFT JOIN X002ec_impo_report_supervisor CAMP_SUPERVISOR ON CAMP_SUPERVISOR.CAMPUS = X002ea_vss_gl_balance_month.CAMPUS
  LEFT JOIN X002ec_impo_report_supervisor ORG_SUPERVISOR ON ORG_SUPERVISOR.CAMPUS = X002ea_vss_gl_balance_month.ORG
;"""
so_curs.execute("DROP TABLE IF EXISTS " + sr_file)
so_curs.execute(s_sql)
so_conn.commit()
