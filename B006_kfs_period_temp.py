...
def kfs_period_list(s_period="curr", s_yyyy=""):
    ...
    *****************************************************************************"""
    ...
    FROM
      X000_Organization
      LEFT JOIN X000_Organization X000_Organization1 ON
        X000_Organization1.FIN_COA_CD = X000_Organization.RPTS_TO_FIN_COA_CD And
        X000_Organization1.ORG_CD = X000_Organization.RPTS_TO_ORG_CD
    ...
    *****************************************************************************"""
    ...
    Select
        CAST(TRIM(UPPER(PUR_VNDR_ADDR_T.VNDR_HDR_GNRTD_ID))||'-'||TRIM(UPPER(PUR_VNDR_ADDR_T.VNDR_DTL_ASND_ID)) AS
            TEXT) VENDOR_ID,
    ...
    s_sql = "CREATE VIEW " + sr_file + " AS " + """
    Select
        CAST(TRIM(UPPER(PUR_VNDR_ADDR_T.VNDR_HDR_GNRTD_ID))||'-'||TRIM(UPPER(PUR_VNDR_ADDR_T.VNDR_DTL_ASND_ID))
            AS TEXT) VENDOR_ID,
        PUR_VNDR_ADDR_T.VNDR_ST_CD,
        PUR_VNDR_ADDR_T.VNDR_CNTRY_CD,
        PUR_VNDR_ADDR_T.VNDR_ADDR_EMAIL_ADDR,
    ...
    Select
        ...
        End as COUNTRY_CD,
        Case
            When X001aa_vendor_rm_address.VNDR_ADDR_EMAIL_ADDR <> '' Then
                Lower(X001aa_vendor_rm_address.VNDR_ADDR_EMAIL_ADDR)
    ...
    *****************************************************************************"""
    ...
    # BUILD CURR APPROVALS ALL APPROVER
    ...
    *****************************************************************************"""
    ...
    print("Build current unique approver list...")
