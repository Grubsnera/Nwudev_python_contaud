...
def vss_lists():
    ...
    *************************************************************************"""
    ...
    From
        STUDYTRANS_CURR TRAN Left Join
        STUDACC STUD ON STUD.KACCID = TRAN.FACCID Left Join
        X000_Transmaster MAST ON MAST.KTRANSMASTERID = TRAN.FTRANSMASTERID Left Join
        X000_Qualifications QUAL On QUAL.KENROLMENTPRESENTATIONID = TRAN.FENROLPRESID Left Join
        X000_Modules MODU On MODU.KENROLMENTPRESENTATIONID = TRAN.FENROLPRESID Left Join
        X004_Bursaries BURS ON BURS.KFINAIDSITEID = TRAN.FFINAIDSITEID Left Join
        SYSTEMUSER USER ON USER.KUSERCODE = TRAN.FAUDITUSERCODE Left Join
        X000_Systemfunction SYSF On SYSF.KSYSTEMFUNCTIONID = TRAN.FAUDITSYSTEMFUNCTIONID
    Order By
        TRAN.TRANSDATETIME
    """
    so_curs.execute("DROP TABLE IF EXISTS " + sr_file)
